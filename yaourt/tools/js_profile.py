...
def _process_shape(shape_df) -> str:
...
def post_process_df_profile(
    ...
    Adds a column for a more explicit event name and adds
    a column for the iteration number.

    :param df: dataframe produced by :func:`js_profile_to_dataframe`
    :param agg: aggregates the result
    :param first_it_out: leaves the first iteration out of the aggregation
    :param agg_op_name: aggregates on operator name or operator index
    :param with_shape: keeps the shape to aggregate
    ...
    agg_df = df[[*agg_cols, "dur"]].groupby(agg_cols).sum()
    return agg_df
...
def js_profile_to_dataframe(
    ...
    :param as_df: returns a DataFrame when True, otherwise a list of dicts
    :param first_it_out: if aggregated, leaves the first iteration out
    :param agg: aggregates by event
    :param agg_op_name: aggregates on operator name or operator index
    :param with_shape: keeps the shape before aggregating
...
def _preprocess_graph1(df: DataFrame):
    """Aggregates profiling data by operator type and provider for bar charts."""
    df = df.copy()
...
def _preprocess_graph2(df: DataFrame) -> DataFrame:
    """Aggregates profiling data by node index for timeline bar charts."""
...
def plot_ort_profile(
    ...
    The input dataframe should be produced by :func:`js_profile_to_dataframe`.

    :param df: dataframe produced by :func:`js_profile_to_dataframe`
    :param ax0: first axis to draw time (created if None)
    :param ax1: second axis to draw occurrences (optional)
    :param title: graph title
    :returns: the axes used for drawing
    """
    fontsize = 10
    if ax0 is None:
    ...
    if "args_provider" in df.columns:
        ...
        with warnings.catch_warnings():
            ...
            ax0.set_yticklabels(ax0.get_yticklabels(), rotation=45, ha="right", fontsize=fontsize)
...
def plot_ort_profile_timeline(
    ...
    :param df: dataframe produced by :func:`js_profile_to_dataframe`
    :param ax: axis to draw (created if None)
    :param iteration: iteration to plot; negative values count from the end
    :param title: graph title
    :param quantile: operators below this quantile of duration are drawn in a
        different colour
    :param fontsize: font size
    :returns: the axis used for drawing
    """
    if ax is None:
        import matplotlib.pyplot as plt
    ...
    if "fence_before" in set(dfi["event_name"]):
        ...
        for irow in dfi.iterrows():
            ...
            if event_name == "fence_before":
                started[op_type, op_name, it] = dict(op_name=op_name, op_type=op_type, begin=ts)
