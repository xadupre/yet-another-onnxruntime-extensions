...
class TestSparseCustomOps(ExtTestCase):
    """Tests for DenseToSparse and SparseToDense custom ops."""

    def _make_dense_to_sparse_model(self, shape: tuple) -> bytes:
    ...
    def _make_sparse_to_dense_model(self) -> bytes:
    ...
    def test_dense_to_sparse_basic(self):
        ...
        model = self._make_dense_to_sparse_model((3, 4))
    ...
    def test_sparse_to_dense_roundtrip(self):
        ...
        shape = (4, 5)
        model_d2s = self._make_dense_to_sparse_model(shape)
        model_s2d = self._make_sparse_to_dense_model()
        sess_d2s = self._make_inference_session(model_d2s)
        sess_s2d = self._make_inference_session(model_s2d)
